...
import { APIConfig, ImageGenerationRequest, ImageGenerationResponse, ErrorResponse } from '../types/index.js';
...
export class ImageGenerationService {
  ...
  private readonly defaultModel = 'MusePublic/489_ckpt_FLUX_1';
  private readonly defaultBaseUrl = 'https://api-inference.modelscope.cn/v1/images/generations';
  ...
  async generateImage(request: ImageGenerationRequest): Promise<string> {
    try {
      ...
      const responseData: ImageGenerationResponse = response.data;
      ...
      if (error.response) {
        const errorMsg = `API请求失败 (状态码: ${error.response.status}): ${
   ...
   * 批量生成图片
   */
  async generateImages(requests: ImageGenerationRequest[]): Promise<Array<{
    prompt: string;
    imageUrl: string;
    error?: string;
  }>> {
