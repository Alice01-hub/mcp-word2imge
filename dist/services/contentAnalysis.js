...
export class ContentAnalysisService {
    ...
    analyzeWebpage(htmlContent) {
        ...
        // 3. 生成建议
        suggestions.push(`分析了网页内容，发现 ${imagePlaceholders.length} 个可能需要图片的位置`, '建议为主要内容区域添加插图来提升用户体验', '可以为产品介绍、功能特性等部分添加相关配图');
        return {
            type: 'webpage',
            imagePlaceholders,
            suggestions
    ...
    analyzeArticle(textContent) {
        ...
        suggestions.push(`在文章的 ${imagePlaceholders.length} 个位置建议添加配图`, '为重要概念和步骤添加示意图可以提升阅读体验', '建议在每个主要章节开头添加题图');
        return {
            type: 'article',
            imagePlaceholders,
            suggestions
     ...
     * 根据上下文生成英文prompt
     */
    generatePromptFromContext(context, alt) {
        // 如果有alt文本，优先使用
        if (alt && alt.length > 0) {
